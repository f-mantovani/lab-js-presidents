...
function getDemocraticPresidents(presidentsArr) {
	return presidentsArr.filter(({ party }) => party === 'Democratic');
}
...
function getAffiliatedPresidents(presidentsArr) {
	return presidentsArr.filter(({ party }) => party !== null);
}
...
function countYearsInOffice(presidentsArr) {
	return presidentsArr.reduce((acc, cV) => {
		if (cV.leftOffice) {
			const addYears = cV.leftOffice - cV.tookOffice;
			return acc + addYears;
		}
		return acc;
	}, 0);
...
function countRepublicanPresidents(presidentsArr) {
	return presidentsArr.reduce((acc, { party }) => (party === 'Republican' ? acc + 1 : acc), 0);
}
...
function getPresidentsBornAfter(presidentsArr, year) {
	return presidentsArr.filter(({ birthYear }) => birthYear > year);
}
